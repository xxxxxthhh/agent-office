  ...
  const registry = new CapabilityRegistry({
    ...
    probe: async ({ command, args }) => {
      ...
      if (command === "claude" && joined === "--help") {
        return {
          // Subcommand probes are gated on the CLI declaring the subcommand, so
          // this mirrors the real help layout rather than a single line.
          stdout: [
            "  --model <model>   Provide an alias (e.g. 'sonnet' or 'opus') or 'claude-sonnet-4-6'.",
            "",
            "Commands:",
