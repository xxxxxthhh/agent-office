...
```js
runTurn({ prompt, workspace, timeoutMs, model, effort })
```

返回标准化的 Turn Protocol 结果和原始输出路径。内置适配器：

- `codex`：调用 `codex exec`，使用 `--output-schema` 和 workspace sandbox。
- `claude`：调用 `claude -p`，使用 `--json-schema` 和结构化 JSON 输出。
