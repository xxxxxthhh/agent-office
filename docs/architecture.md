...
- 同时还持有一份按工作区绝对路径哈希的租约：只有任务级租约时，两个**不同**任务共享同一工作区仍可并发写入，串行保证会失效；
...
`AbortSignal` 从编排器穿过适配器传到子进程。子进程以独立进程组启动，中断时向**整个进程组**先 `SIGTERM`、0.5 秒后 `SIGKILL`，并**等待进程真正退出**后才释放租约、把任务置回 `ready`——代理会派生自己的 shell 与工具，提前释放租约会让下一次运行与残留后代同时写同一批文件。被中断的代理**不会**被标记为 `failed`——中断是用户决定，不是代理错误，因此已完成的回合、消息和产物全部保留。
