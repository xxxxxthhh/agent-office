...
- 同时还持有一把放在**工作区根目录**（`<workspace>/.agent-office.lock`，路径经 realpath 规范化，原子独占创建）的工作区锁：只有任务级租约时，两个**不同**任务、乃至两份不同 `stateDir` 的配置共享同一工作区仍可并发写入，串行保证会失效；锁必须放在所有配置都可见的位置，`stateDir` 不满足这一点；
- 同机租约**只以进程存活为过期依据**（心跳暂停但进程存活——如被 SIGSTOP——不视为过期，否则恢复后即双写者）；跨主机以心跳为准。可接管的租约经由原子 mkdir 互斥 + 原地 rename 替换 + 回读校验接管，从不"删除再创建"；
...
`AbortSignal` 从编排器穿过适配器传到子进程。子进程以独立进程组启动，中断时向**整个进程组**先 `SIGTERM`、0.5 秒后 `SIGKILL`，并**轮询到进程组消失**后才释放租约、把任务置回 `ready`——直接子进程的 `close` 不能证明孙进程已退出（Windows 用 `taskkill /T`，无组探测，保证为尽力而为）——代理会派生自己的 shell 与工具，提前释放租约会让下一次运行与残留后代同时写同一批文件。被中断的代理**不会**被标记为 `failed`——中断是用户决定，不是代理错误，因此已完成的回合、消息和产物全部保留。
